...
const noParamReassign = {
  ...
  create(context) {
    ...
    function isModifyingProp(reference) {
      ...
      while (
        parent &&
        (!stopNodePattern.test(parent.type) || parent.type === 'ForInStatement' || parent.type === 'ForOfStatement')
      ) {
        switch (parent.type) {
          // e.g. foo.a = 0;
    ...
    function checkVariable(variable) {
      ...
      if (def.type === 'Parameter') {
        ...
        if (/^(set|each|handle|change)|(change|handler?)$/i.test(id?.name)) {
          // 函数名以 each 或者 handle 开头的，不校验
          return;
        }
        if (
          def?.node?.type === 'FunctionExpression' &&
          /^(set|each|handle|change)|(change|handler?)$/i.test(def?.node?.parent?.key?.name)
        ) {
          return;
        }
        if (callee?.type === 'MemberExpression') {
          if (callee?.property?.name === 'forEach') {
            // 数组的 forEach 方法的回调函数，不校验
            return;
          }
          if (def.index === 0 && ['reduce', 'reduceRight'].includes(callee?.property?.name)) {
            // reduce, reduceRight 的第一个参数，不校验
            return;
          }
        }
...
export default noParamReassign;
